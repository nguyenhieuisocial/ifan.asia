-- #45 — Sửa khối "Khách nóng" của màn "Hôm nay gọi ai" loại nhầm khách VĨNH VIỄN
--
-- Bệnh: CTE `hot` trong today_queue loại khách có BẤT KỲ activity nào chưa xong
-- (done_at is null) — kể cả ghi chú/cuộc gọi không có hạn. Một khách nóng chỉ cần
-- dính một ghi chú không hạn là biến mất khỏi khối "Khách nóng" mãi mãi (ghi chú
-- không bao giờ quá hạn nên cũng không bao giờ xuất hiện ở khối quá hạn/hôm nay).
-- Nghĩa đúng phải là: "không có việc CÓ HẠN đang chờ" → thêm `a.due_at is not null`.
-- Cùng nghĩa với tập việc mà SLA quét (#33) và khối quá hạn của chính hàm này (CTE act).
--
-- ⚠️ CẢNH BÁO REGRESSION: thân hàm dưới đây CHÉP TỪ BẢN MỚI NHẤT (#31 — đã gồm
-- sửa `unans` của #30/#31 và tra tên qua conversation_contact_name). KHÔNG chép
-- lại từ #16. Ai sửa hàm này sau: phải chép từ bản mới nhất, chỉ đổi đúng chỗ cần.
-- Khác #31 đúng 2 chỗ: (1) `and a.due_at is not null` trong not exists của `hot`;
-- (2) search_path ghim `pg_temp` cuối (giữ chốt #40 — create or replace ghi đè
-- cấu hình cũ, chép nguyên `public` là tự tháo chốt).
