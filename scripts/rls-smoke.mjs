...
try {
  ...
  // ==========================================================================
  // MỌI VIEW PHẢI BẬT security_invoker
  // ==========================================================================
  //
  // Mặc định của PostgreSQL: view chạy bằng quyền **CHỦ SỞ HỮU view**, nên RLS
  // của bảng gốc KHÔNG được áp dụng. Chính sách cách ly tiệm vẫn nằm nguyên
  // trên bảng — nó chỉ bị đi vòng qua.
  //
  // Đo 21/08, với một người dùng thuộc ĐÚNG MỘT tiệm:
  //     loyalty_balances → 1.061 dòng của 6 TIỆM
  //     loyalty_debt     →     6 dòng của 6 TIỆM
  // Tức chủ tiệm A đọc được điểm tích của từng khách ở tiệm B.
  //
  // ⚠️ Vì sao 662 ca trước đó không bắt: chúng quét **BẢNG**, và bảng gốc thì
  // đúng chính sách. VIEW là một tầng khác, và tầng đó chưa từng có gì canh —
  // đúng cùng một chỗ mù với tầng HÀM ngay bên dưới.
  console.log("[rls-smoke] Kiểm tra security_invoker của view:");
  {
    const { rows: v } = await c.query(`
      select c.relname
       ...
       where n.nspname = 'public' and c.relkind in ('v','m')
         ...
         -- PostgreSQL lưu giá trị này là 'on' khi khai lúc CREATE và 'true'
         -- khi đặt bằng ALTER — cùng nghĩa. Chỉ so với 'on' là báo oan đúng
         -- ba view vừa vá xong, và một cổng kêu oan là một cổng sắp bị tắt.
         and coalesce((select option_value from pg_options_to_table(c.reloptions)
                        where option_name = 'security_invoker'), 'off')
             not in ('on', 'true')
       order by 1`);
    check("Mọi view authenticated đọc được đều bật security_invoker", v.length === 0,
      `còn thiếu: ${v.map((r) => r.relname).join(", ")}`);
  }
  ...
  console.log("[rls-smoke] Xoá tiệm không bị nhật ký bản ghi chặn (migration #82):");
  {
    ...
    /**
     * ⚠️ PHẢI CÓ PHIẾU QUỸ, không chỉ có khách. Ca này TRƯỚC ĐÂY chỉ gieo một
     *   khách hàng — nên nó xanh suốt trong khi đường xoá tiệm thật ra ĐÃ HỎNG.
     *
     *   Chuyện đã xảy ra: #302 (21/08) cấm xoá cứng dòng sổ quỹ, KHÔNG ngoại lệ
     *   — đúng, vì sổ tiền mà xoá được dòng thì nó thôi là sổ. Nhưng #270 lại
     *   dựa vào chính đường xoá đó ("xoá cứng một phiếu quỹ chỉ xảy ra khi xoá
     *   tiệm"). Hai luật đúng, đặt cạnh nhau thành một cửa khoá kín: từ 21/08,
     *   **tiệm nào có dù chỉ một phiếu quỹ là không xoá được nữa**. Vá ở #367.
     *
     *   Bài học cho chính ca kiểm này: gieo một bảng con là chưa đủ. Phải gieo
     *   bảng con nào có CHỐT CHẶN GHI mạnh nhất — vì đó mới là thứ chặn cascade.
     */
    await c.query(
      `insert into public.cash_entries (tenant_id, direction, amount_vnd, category, fund)
         values ($1, 'in', 1000, 'other_in', 'cash')`, [tD.id]);
    ...
    const leftCash = await c.query(
      `select count(*)::int as n from public.cash_entries where tenant_id = $1`, [tD.id]);
    check("Xoá tiệm CÓ PHIẾU QUỸ cũng sạch (chốt cấm xoá sổ quỹ không chặn nhầm)",
      !delErr && leftCash.rows[0].n === 0, `còn ${leftCash.rows[0].n} phiếu`);
