-- ════════════════════════════════════════════════════════════════════
-- #350 — DẤU THẬP PHÂN TRONG CÂU CHỮ BÁO ĐỘNG PHẢI LÀ DẤU PHẨY
-- ════════════════════════════════════════════════════════════════════
-- Chạy thử thật 22/08 trên ba tiệm mẫu, câu ra là "Thường ngày 18.5 lịch" —
-- đúng số, sai chính tả tiếng Việt (phải là "18,5").
--
-- ⚠️ CHỖ NÀY CHỈ LÀ BẢN DỰ PHÒNG, VÀ VẪN PHẢI ĐÚNG. Bản web dựng câu từ khoá
--   dịch (`title_key`/`body_key`/`params`) nên ICU tự đặt dấu theo ngôn ngữ
--   người đang xem; cột `title`/`body` chỉ dùng khi bản web CŨ HƠN nguồn ghi và
--   chưa biết khoá đó. Nhưng "chỉ dùng khi..." không có nghĩa là "không ai
--   đọc" — đúng lúc rơi vào bản dự phòng là đúng lúc không ai kịp sửa.
--
-- Vì sao mức nền lại lẻ: nó là TRUNG VỊ của 14 ngày. Số ngày chẵn thì trung vị
-- là trung bình hai giá trị giữa, nên ".5" là chuyện thường xuyên, không phải
-- ca hiếm.
  ...
  loop
    ...
    if v_gio = c_gio_huy then
      ...
      if v_huy >= c_huy_toi_thieu and v_huy >= 2 * v_tv and v_huy >= v_cao then
        ...
        -- Dấu thập phân tiếng Việt là DẤU PHẨY. Chuỗi này chỉ là bản dự phòng
        -- (bản web mới dựng câu từ khoá dịch), nhưng bản dự phòng cũng là chữ
        -- người Việt đọc — "18.5 lượt" là sai chính tả tiếng Việt.
        v_tv_chu := replace(v_tv::text, '.', ',');

        if v_khung.n * 2 >= v_huy then
          v_khoa_th := 'batThuong.huy.bodyKhungGio';
          v_than := 'Mức thường ngày là ' || v_tv_chu || ' lượt. Phần lớn rơi vào khung '
          ...
          v_than := 'Mức thường ngày là ' || v_tv_chu || ' lượt.';
        end if;
      ...
      if v_moc >= c_lich_toi_thieu and v_mai * 2 <= v_moc then
        ...
        v_than := 'Thường ngày ' || replace(v_moc::text, '.', ',') || ' lịch. Còn kịp gọi lại khách cũ hoặc chạy ưu đãi trong tối nay.';
