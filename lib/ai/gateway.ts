...
/**
 * ⚡ ĐÃ GỠ `rewriteNotification` (Haiku soạn lại tin chuông founder trước khi
 * gửi — từng là ADR-0007 mục 12e, thi công 14/08).
 *
 * Gỡ ngày 17/08 sau khi founder chốt không cắm khoá AI vào máy chủ. Không phải
 * chỉ vì tiền — hai căn cứ đo được: bước này CHƯA TỪNG chạy một lần nào trên
 * máy chủ (không có `ANTHROPIC_API_KEY` ở production, và 60/60 tin đều gửi
 * nguyên bản gốc), và thử thật cho thấy nó KHÔNG chữa được ca hỏng 17/08 — nó
 * chỉ thêm dấu vào một câu chỉ đạo bị chép nhầm vào commit, không thể suy ra
 * "người dùng được gì" từ thông tin không có trong đầu vào.
 *
 * Lý do đầy đủ + phương án thay thế (hook `commit-msg` + lưới đỡ ở
 * `tg_release_mark`): đầu `supabase/migrations/20260817000129_founder_line_guard.sql`
 * và `lib/notify/platform-outbox.ts`.
 *
 * ⚠️ Đừng dựng lại nó chỉ vì có khoá AI: giới hạn ở căn cứ thứ hai là giới hạn
 * về THÔNG TIN, không phải về giá. Muốn dựng lại thì phải đổi đầu vào (cho AI
 * đọc cả diff, không chỉ câu commit).
 */
