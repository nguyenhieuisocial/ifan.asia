...
**LUẬT NÀY ĐÃ BỊ PHÁ MỘT LẦN — ghi ra để không lặp (21/08).** Màn Sự kiện marketing (V8) làm đúng cái điều đoạn trên cấm: bấm "Gửi cho N người" rồi hiện **"Đã gửi xong — số thật: N người"**, trong khi `guiTin` chỉ ghi vào hai bảng và dừng — không một khách nào nhận được gì. Hại gấp đôi vì trigger `campaign_recipient_dong_dau` đóng dấu `marketing_last_sent_at`, nên N người ấy bị khoá 7 ngày khỏi đợt sau: **mất cửa sổ liên lạc cho một tin chưa hề gửi**.

Vì sao lọt: luật chỉ nằm trong đoạn văn về màn Chốt lịch, không nằm ở chỗ nào người xây màn KHÁC phải đọc — và mục "khoảng chưa làm" của tài liệu này KHÔNG liệt kê nó. Một luật chỉ được ghi cạnh màn đã tuân thủ thì không bảo vệ được màn dựng sau.

Đã sửa: đổi chữ cho đúng việc màn làm (**chốt danh sách**, không phải gửi), thêm khối nói thẳng iFan chưa nhắn hộ được + nút **chép danh sách số đã lọc sạch** và **chép nội dung tin** để chủ tiệm tự nhắn bằng Zalo của mình, và nói rõ dấu 7 ngày. Ngày Zalo OA được duyệt thì nối đường gửi vào đúng chỗ đã đánh dấu trong `app/app/events/actions.ts`.

**Luật rút thành câu ngắn cho mọi màn sau này:** *chưa có đường giao thì không được viết chữ "đã gửi"* — kể cả khi bảng dữ liệu đã ghi xong. Việc ghi vào bảng là việc của máy; "đã gửi" là lời hứa với người.
